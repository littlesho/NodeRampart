...
class PackagingTests(unittest.TestCase):
    ...
    def prepare_rpm_source(self, version="0.4.0-alpha.3"):
    ...
    def test_candidate_debian_mapping_and_native_order(self):
        self.write(self.project / "VERSION", "0.4.0-alpha.3\n")
        result = self.run_script("scripts/build-deb.sh")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("Version: 0.4.0~alpha.3\n", (self.lab / "deb-control").read_text())
        self.assertEqual((self.lab / "deb-output").read_text(), "noderampart_0.4.0-alpha.3_amd64.deb")
        if shutil.which("dpkg"):
            for lower, higher in (("0.4.0~alpha.2", "0.4.0~alpha.3"), ("0.4.0~alpha.3", "0.4.0")):
    ...
    def test_candidate_rpm_mapping_preserves_full_program_version(self):
        ...
        for expected in ("%global noderampart_version 0.4.0-alpha.3", "Version:        0.4.0",
                         "Release:        0.alpha.4%{?dist}", "Source0:        %{name}-%{noderampart_version}.tar.gz",
        ...
        self.assertTrue(all(name.split('/')[0] == 'NodeRampart-0.4.0-alpha.3' for name in names))

    @unittest.skipUnless(shutil.which("rpm"), "native RPM tooling is unavailable")
    def test_candidate_native_rpm_order(self):
        for fedora in (43, 44):
            for lower, higher in (("0.alpha.3", "0.alpha.4"), ("0.alpha.4", "1")):
    ...
    def test_debian_arm64_package_checks_binary_architecture(self):
        ...
        self.assertIn("Version: 0.4.0~alpha.3\n", (self.lab / "deb-control").read_text())
        self.assertEqual((self.lab / "deb-output").read_text(), "noderampart_0.4.0-alpha.3_arm64.deb")
