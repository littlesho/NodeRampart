...
cd "$PROJECT_DIR"
VERSION=$(tr -d '\n' < VERSION)
[ "$VERSION" = 0.4.0-alpha.3 ] || { echo 'release tooling currently targets 0.4.0-alpha.3' >&2; exit 1; }
...
try:
    ...
    (stage / 'release.json').write_text(json.dumps({'format': 1, 'version': '0.4.0-alpha.3',
