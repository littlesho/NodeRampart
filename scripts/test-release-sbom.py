...
class SBOMTests(unittest.TestCase):
    ...
    def test_source_rpm_and_arbitrary_package_names_are_rejected(self):
        for name in ('noderampart-0.4.0-0.alpha.4.fc44.src.rpm', 'other_0.4.0~alpha.3_amd64.deb',
                     'noderampart_0.4.0-alpha.3_amd64.deb/../escape', 'noderampart_0.4.0-alpha.3_i386.deb'):
    ...
    def test_wrong_tool_digest_and_existing_outputs_are_rejected(self):
        ...
        package = self.root / 'noderampart_0.4.0-alpha.3_amd64.deb'
    ...
    def test_public_names_native_versions_and_uploaded_set(self):
        ...
        for arch in ('amd64', 'arm64'):
            name = f'noderampart_0.4.0-alpha.3_{arch}.deb'
            self.assertIn(name, release.runtime_packages())
            self.assertEqual(release.package_identity(name),
                             {'name': 'noderampart', 'version': '0.4.0~alpha.3', 'architecture': arch})
    ...
    def test_valid_deb_filename_does_not_override_native_identity(self):
        ...
        for values in (('other', '0.4.0~alpha.3', 'amd64'),
                       ('noderampart', '0.4.0-alpha.3', 'amd64'),
                       ('noderampart', '0.4.0~alpha.1', 'amd64'),
                       ('noderampart', '0.4.0~alpha.3', 'arm64')):
