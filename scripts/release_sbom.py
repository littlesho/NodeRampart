...
def package_arch(name):
    match = re.fullmatch(r"noderampart_0\.4\.0-alpha\.5_(amd64|arm64)\.deb", name)
    if match:
        return match[1]
    match = re.fullmatch(r"noderampart-0\.4\.0-0\.alpha\.6\.fc(43|44)\.(x86_64|aarch64)\.rpm", name)
    if match:
...
def runtime_packages():
    ...
    names.update(f"noderampart-0.4.0-0.alpha.6.fc{fedora}.{arch}.rpm"
...
def package_identity(name):
    ...
    if name.endswith('.deb'):
        return {"name": "noderampart", "version": "0.4.0~alpha.5", "architecture": arch}
    match = re.fullmatch(r"noderampart-(0\.4\.0-0\.alpha\.6\.fc(?:43|44))\.(x86_64|aarch64)\.rpm", name)
...
def release_assets():
    ...
    return (packages | {name + suffix for name in packages for suffix in ('.spdx.json', '.buildinfo.json')}
            | {'noderampart-0.4.0-0.alpha.6.fc44.src.rpm', 'bootstrap.sh', 'release.json', 'SHA256SUMS'})
...
def inspect_package(package, destination):
    ...
    if package.suffix == ".deb":
        ...
        if actual != ["noderampart", "0.4.0~alpha.5", arch]:
